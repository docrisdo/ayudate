...
export default function SpeechProvider({ children }) {
  ...
  const play = useCallback((text, transient = false, contextual = false) => {
    ...
    try {
      ...
      setStatus('idle')
      if (!contextual) setError('No pudimos iniciar la lectura. Intenta pulsar Leer de nuevo.')
    ...
    function done(ok) {
      ...
      current.current = null
      // The finishing utterance removes these controls. Keep keyboard focus
      // on the invoking control (or inside the active dialog) before removal.
      if (ok && !suspended.current && document.activeElement?.closest('.speech-controls')) {
        const dialog = document.querySelector('dialog[open]')
        const origin = returnFocus.current
        const target = origin?.isConnected && (!dialog || dialog.contains(origin))
          ? origin : dialog?.querySelector('button:not(:disabled)') || document.querySelector('#root h1')
    ...
    utterance.onerror = event => {
      ...
      if (!contextual && !transient && !['canceled', 'interrupted'].includes(event.error)) setError('No pudimos iniciar la lectura. Intenta pulsar Leer de nuevo.')
    }
    setError('')
    setStatus('speaking')
