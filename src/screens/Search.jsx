...
function ProductPhoto({ product }) {
  ...
  return <span className="sp-photo">{image ? <img src={image} alt="" /> : <Icon name="icon-bolsa" />}</span>
}
...
export default function Search({ products, query, onQuery, category, onCategory, selectedList, cart, onAddToList, onUndoList, onAddToCart, onUndoCart, onNavigate, onRead, voiceSupported }) {
  ...
  const codeButton = useRef(null)
  const inputRef = useRef(null)
  const resultRef = useRef(null)
  const results = searchCatalog(products, query, category, sort)
  useEffect(() => { const timer = setTimeout(() => announce(`${results.length} resultados${query.trim() ? ` para ${query.trim()}` : ''}.`), 400); return () => clearTimeout(timer) }, [query, category, results.length, announce])
  ...
  useVoiceScope('search', command => {
    if (codeOpen && ['readProduct', 'addCart', 'addList', 'location', 'selectProduct'].includes(command.type)) return { message: 'Cierra el escáner antes de utilizar comandos sobre los productos.' }
    if (command.type === 'search') { setSelected(null); return null }
    if (command.type === 'selectProduct') {
      const product = results[command.index]
      if (!product) return { message: 'No hay un producto en esa posición.' }
      setSelected(product.id)
      return { read: productSpeech(product) }
    }
    if (!['readProduct', 'addCart', 'addList', 'location'].includes(command.type)) return null
    const product = results.find(item => item.id === selected)
    if (!product) return { message: 'Primero selecciona un producto.' }
    if (command.type === 'readProduct') return { read: productSpeech(product) }
    if (command.type === 'location') { setSelected(product.id); return { message: `Pasillo ${product.aisle}, ${product.shelf}.` } }
    if (!product.available) return { message: 'Este producto está agotado.' }
    if (command.type === 'addCart') { addCart(product); return { message: `${product.name} agregado al carrito.` } }
    if (!selectedList) return { message: 'Primero crea una lista en Mis listas.' }
    if (selectedList.items.includes(product.id)) return { message: 'Este producto ya está en tu lista.' }
    addList(product)
    return { message: `${product.name} agregado a tu lista.` }
  })
  function closeScanner() {
    setCodeOpen(false)
    requestAnimationFrame(() => codeButton.current?.focus())
  }
  function foundCode(product) {
  ...
  return <div className="lists-screen sp-screen">
    ...
    {codeOpen && <BarcodeScanner products={products} onFound={foundCode} onClose={closeScanner} />}
    <main className="sp-content" id="contenido-principal" tabIndex={-1}>
      ...
      <section className="sp-filters" aria-label="Buscar y filtrar productos">
        <div className="sp-search-row"><form className="sp-search" onSubmit={event => { event.preventDefault(); resultRef.current?.focus(); }} role="search"><Icon name="icon-buscar" /><label className="a11y-sr-only" htmlFor="sp-search-input">Buscar producto</label><input id="sp-search-input" ref={inputRef} type="search" placeholder="Producto, categoría o pasillo" value={query} onChange={event => { onQuery(event.target.value); setSelected(null); setMessage('') }} /><button className="sp-clear" type="button" aria-label="Limpiar búsqueda" disabled={!query} onClick={() => { onQuery(''); inputRef.current?.focus() }}>×</button><button className="lists-button" type="submit">Buscar</button></form>
          <button className="sp-code" ref={codeButton} aria-haspopup="dialog" aria-expanded={codeOpen} onClick={() => setCodeOpen(true)}><Icon name="icon-barcode" /><span><strong>Buscar por código</strong><small>Busca un producto con su código</small></span></button>
        </div>
        <div className="sp-categories" aria-label="Filtrar por categoría">{categories.map(([name, icon]) => <button key={name} aria-pressed={category === name} onClick={() => filter(name)}>{icon && <Icon name={icon} />}{name === 'Frutas' ? 'Frutas y verduras' : name}</button>)}<label className="a11y-sr-only" htmlFor="sp-more-categories">Más categorías</label><select id="sp-more-categories" value={categories.some(([name]) => name === category) ? '' : category} onChange={event => filter(event.target.value || 'Todos')}><option value="">Más categorías</option><option value="Cereales">Cereales</option></select></div>
      </section>
      ...
      <div className="sp-results">{results.map(product => <article className={`sp-result ${selected === product.id ? 'sp-selected' : ''}`} key={product.id}>
        <div className="sp-product"><h3 className="sp-product-heading"><button className="sp-select-product" aria-label={`Seleccionar ${product.name} ${product.unit} ${product.brand}`} aria-expanded={selected === product.id} onClick={() => setSelected(selected === product.id ? null : product.id)}><ProductPhoto product={product} /><span><strong id={`sp-product-${product.id}`}>{product.name} {product.unit}</strong><small>{product.brand}</small></span></button></h3><button className="sp-read" aria-label={`Leer información de ${product.name} ${product.brand}`} disabled={!voiceSupported} onClick={() => onRead(productSpeech(product))}><Icon name="nav-leer" />Leer</button></div>
        ...
        <div className="sp-actions"><button className="sp-add-list" aria-label={`${selectedList?.items.includes(product.id) ? 'En tu lista' : 'Agregar a lista'}: ${product.name} ${product.brand}`} disabled={!product.available || !!selectedList?.items.includes(product.id)} onClick={() => addList(product)}><Icon name="icon-listas" />{selectedList?.items.includes(product.id) ? 'En tu lista' : 'Agregar a lista'}</button><button className="lists-button" aria-label={`Agregar al carrito: ${product.name} ${product.brand}`} disabled={!product.available} onClick={() => addCart(product)}><Icon name="icon-carrito" />Agregar al carrito</button><button className="sp-location" aria-label={`Ver ubicación de ${product.name} ${product.brand}`} aria-expanded={selected === product.id} aria-controls={`sp-detail-${product.id}`} onClick={() => setSelected(selected === product.id ? null : product.id)}><Icon name="icon-navegacion" />Ver ubicación</button></div>
        {selected === product.id && <section className="sp-detail" id={`sp-detail-${product.id}`} aria-label={`Detalle de ${product.name} ${product.brand}`}><p><strong>{product.name}{product.brand && ` · ${product.brand}`}</strong> — Pasillo {product.aisle}, {product.shelf}. {product.available ? 'Disponible' : 'Agotado'}. Código del catálogo: {product.id}.</p><div><button disabled={!voiceSupported} onClick={() => onRead(productSpeech(product))}><Icon name="nav-leer" />Leer producto</button><button onClick={() => onNavigate('route')}><Icon name="icon-ruta" />Ir a Mi ruta</button><button onClick={() => { setSelected(null); document.querySelector(`[aria-controls="sp-detail-${product.id}"]`)?.focus() }}>Cerrar detalle</button></div></section>}
      </article>)}</div>
