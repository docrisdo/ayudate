...
export default function HomeScreen({ accompanimentEnabled, contextSummary, voiceSupported, pictogramsEnabled, listCount, activeList, listProducts, cart, currentStop, onDisableAccompaniment, onNavigate, onRead }) {
  const { count: found } = routeProgress(listProducts, activeList, cart)
  ...
  return (
    <div className="home-screen">
      ...
      <main className="home-content" id="contenido-principal" tabIndex={-1}>
        <section className="home-hero" aria-labelledby="home-title">
          <div className="home-hero-copy">
            <h1 id="home-title"><span className="a11y-sr-only">Inicio. </span>Hola, <span>¿qué necesitas hoy?</span></h1>
        ...
        <section className="home-continue" aria-labelledby="continue-title">
          ...
          <p className="home-context-summary">{contextSummary}</p>
        ...
        <section className="home-access" aria-labelledby="access-title">
          ...
          <div className="home-voice-toggle"><AyudateIcon name="voice" /><span>Acompañamiento por voz: {accompanimentEnabled ? 'activado' : 'apagado'}</span><button type="button" onClick={() => accompanimentEnabled ? onDisableAccompaniment() : onNavigate('accessibility')}>{accompanimentEnabled ? 'Desactivar acompañamiento' : 'Personalizar acompañamiento'}</button></div>
