...
const options = [
  ...
  ['accessibleRoute', 'Ruta accesible', 'Prioriza recorridos más cómodos y con menos obstáculos durante tu compra.', 'icon-ruta'],
  ['bigButtons', 'Botones simplificados', 'Muestra acciones más grandes, claras y fáciles de utilizar.', 'icon-botones-simplificados'],
  ['pictograms', 'Apoyo con imágenes y pictogramas', 'Añade apoyos visuales para identificar productos, secciones y acciones.', 'icon-apoyo-imagenes'],
]
...
export default function Accessibility({ preferences: savedPreferences, accompaniment, product, onNavigate, onRead, voiceSupported, onSave, onCancel, initialFlow }) {
  ...
  const appClass = `app-shell${preferences.largeText ? ' large-text' : ''}${preferences.highContrast ? ' high-contrast' : ''}${preferences.bigButtons ? ' big-buttons' : ''}`
      ...
      <div className="ac-columns">
        ...
        <section className="ac-panel ac-preview" aria-labelledby="ac-preview-title"><div className="ac-preview-heading"><Icon name="icon-ojo" /><div><h2 id="ac-preview-title">Vista previa</h2><p>Así se verá la información con tus preferencias activas.</p></div></div>
          <div className="ac-preview-card"><div className="ac-product"><img className="ac-product-photo" src={product.image || assets.products.leche} alt="" /><div><h3>{product.name} {product.unit}</h3><p>Marca {product.brand}</p><strong className="ac-aisle">{preferences.pictograms && <Icon name="icon-navegacion" />}Pasillo {product.aisle}</strong><strong className="ac-price">${product.price.toFixed(2)}</strong><span className="ac-available">{preferences.pictograms && <Icon name="icon-check" />}{product.available ? 'Disponible' : 'Agotado'}</span></div><button className="ac-product-read" aria-label="Leer producto de vista previa" disabled={!voiceSupported} onClick={() => onRead(productMessage)}><Icon name="icon-audio" /></button></div>
            ...
            <div className="ac-cart-notice"><Icon name="icon-check" /><div><strong>Vista previa de tus preferencias</strong><p>Puedes revisar tus preferencias sin modificar tu compra.</p></div></div>
          </div>
        </section>
      </div>
