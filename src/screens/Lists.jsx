...
function ProductImage({ product }) {
  ...
  const image = product.image || missingProductPhotos[product.id] || productImages[product.id]
  return image ? <img className="lists-product-image" src={image} alt="" width="52" height="44" /> : <span className="lists-product-image" aria-hidden="true" />
}
...
export default function ListsScreen({ lists, selectedList, products, newListName, onNameChange, onCreateList, onNewList, onSelectList, onUpdateList, onDeleteList, onRestoreList, onAddProduct, onRemoveProduct, onStart, onNavigate, onRead, voiceSupported }) {
  ...
  return (
    <div className="lists-screen">
      ...
      <main className="lists-content" id="contenido-principal" tabIndex={-1}>
        ...
        <div className="lists-columns">
          <section className="lists-saved-panel" aria-labelledby="saved-title">
            ...
            <div className="lists-saved-stack">
              {lists.map(list => <article key={list.id} className={`lists-saved-card ${selectedList?.id === list.id ? 'is-selected' : ''}`}>
                <button className="lists-card-select" type="button" aria-label={`Seleccionar ${list.name}`} aria-describedby={`lists-count-${list.id}`} aria-pressed={selectedList?.id === list.id} onClick={() => openList(list)}>
                  <Icon name={listIcon(list)} className="lists-list-symbol" /><span><strong>{list.name}</strong><span id={`lists-count-${list.id}`}>{list.items.length} {list.items.length === 1 ? 'producto' : 'productos'}</span><small>{updatedLabel(list)}</small></span>
                </button>
                <div className="lists-card-actions">
                  ...
                  <button type="button" aria-label={`Eliminar ${list.name}`} aria-expanded={deleteId === list.id} onClick={() => { setDeleteId(deleteId === list.id ? null : list.id); requestAnimationFrame(() => document.querySelector('.lists-delete-confirm button')?.focus()) }}><Icon name="icon-basura" /><span>Eliminar</span></button>
                </div>
                ...
                {deleteId === list.id && <div className="lists-delete-confirm" role="group" aria-label={`¿Eliminar ${list.name}?`} onKeyDown={event => { if (event.key === 'Escape') { setDeleteId(null); event.currentTarget.closest('article').querySelector('.lists-card-actions button:last-child').focus() } }}><span>¿Eliminar «{list.name}»?</span><button type="button" onClick={() => removeList(list)}>Eliminar lista</button><button type="button" onClick={event => { setDeleteId(null); event.currentTarget.closest('article').querySelector('.lists-card-actions button:last-child').focus() }}>Cancelar</button></div>}
              </article>)}
              {!lists.length && <p className="lists-empty">Aún no tienes listas. Usa «Nueva lista» para preparar tu compra.</p>}
            </div>
            {deleted && <div className="lists-undo" role="status"><span>Se eliminó «{deleted.list.name}».</span><button type="button" onClick={() => { onRestoreList(deleted.list, deleted.index); announce(`Lista ${deleted.list.name} restaurada.`); setDeleted(null); requestAnimationFrame(() => document.querySelector(".lists-card-select")?.focus()) }}>Deshacer</button></div>}
          </section>
          ...
          <section className="lists-detail-panel" aria-labelledby="lists-detail-title" ref={detailRef} tabIndex={-1}>
            ...
            {editing && selectedList && <form className="lists-rename" onSubmit={event => { event.preventDefault(); if (draftName.trim()) { onUpdateList({ ...selectedList, name: draftName.trim(), updatedAt: new Date().toISOString() }); announce(`Lista ${draftName.trim()} guardada.`) } }}><label htmlFor="lists-edit-name">Nombre de la lista</label><div><input id="lists-edit-name" ref={nameRef} value={draftName} onChange={event => setDraftName(event.target.value)} required maxLength={80} /><button className="lists-button" type="submit">Guardar</button><button type="button" onClick={() => { setEditing(false); detailRef.current?.focus() }}>Listo</button></div></form>}
            <h3 className="lists-products-title">Productos de esta lista</h3>
            <ul className="lists-products">
              {selectedProducts.map(product => <li key={product.id}><ProductImage product={product} /><span>{product.name}</span><span className="lists-product-unit">{product.unit}</span>{editing && <button type="button" aria-label={`Quitar ${product.name}`} onClick={event => { focusAfterRemoval(event.currentTarget, 'li', 'button', '.lists-detail-edit'); onRemoveProduct(product.id); announce(`${product.name} eliminado de la lista.`) }}><Icon name="icon-basura" /></button>}</li>)}
            </ul>
