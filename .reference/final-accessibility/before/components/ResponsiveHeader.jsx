...
export default function ResponsiveHeader({ children, active, onNavigate, onRead, voiceSupported }) {
  ...
  useEffect(() => {
    if (!open) return
    header.current?.querySelector('nav button')?.focus()
  ...
  return <div className="responsive-header">
    ...
    <header className="mobile-header" ref={header}>
      ...
      <nav id={menuId} className="mobile-header-menu" aria-label="Navegación principal" hidden={!open}>{navigation.map(([id, label]) => <button key={id} aria-current={active === id || (active === 'newlist' && id === 'lists') ? 'page' : undefined} onClick={() => { if (id === active) toggle.current?.focus(); navigate(id) }}>{label}{(active === id || (active === 'newlist' && id === 'lists')) && <span aria-hidden="true">Actual</span>}</button>)}</nav>
    </header>
  </div>
}
