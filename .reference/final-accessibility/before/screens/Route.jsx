...
export default function Route({ list, products, index, onIndex, cart, onFound, onRestore, onNavigate, onRead, voiceSupported }) {
  ...
  return <div className="lists-screen rt-screen">
    ...
    <main className="rt-content" id="main-content" tabIndex={-1}>
      <section className="rt-hero" aria-labelledby="rt-title"><div><h1 id="rt-title">Tu <span>ruta</span></h1><p>Hemos organizado tu lista para reducir recorridos innecesarios.</p></div><div className="rt-banner" aria-hidden="true"><p>Compras más<br />simples, días<br />más fáciles ♡</p><img src={assets.illustrations.productos_frescos} alt="" /></div></section>
      {!products.length ? <section className="rt-panel rt-empty"><Icon name="icon-ruta" /><h2>Prepara tu recorrido</h2><p>Selecciona una lista con productos para organizar tu ruta de compra.</p><button className="lists-button" onClick={() => onNavigate('lists')}>Ir a Mis listas</button></section> : <div className="rt-columns"><section className="a11y-sr-only" aria-label="Recorrido en texto"><h2>Recorrido de compra</h2><p role="status" aria-atomic="true">Zona actual: {currentZone}. {instruction} Siguiente zona: {upcoming[0] || 'Caja'}.</p><h3>Productos pendientes</h3><ul>{pending.map(item => <li key={item.id}>{item.name}, {item.unit}. Pasillo {item.aisle}, {item.shelf}. {!item.available && 'Agotado.'}</li>)}</ul></section>
        ...
        <aside className="rt-sidebar">
          ...
          <section className="rt-panel rt-stop" ref={detailRef} tabIndex={-1} aria-labelledby="rt-stop-title">
            ...
            {undo && <div className="rt-found-note" role="status"><span>Producto localizado y añadido al carrito.</span><button onClick={() => { onRestore(undo.id, undo.quantity); setUndo(null) }}>Deshacer</button></div>}
