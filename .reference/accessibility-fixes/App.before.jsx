...
import { initialWelcomeScreen, supportPreferences } from './screens/welcomeFlow.js'
...
function App() {
  const [screen, setScreen] = useState(() => initialWelcomeScreen(readStorage('ayudate-onboarding-stage', null), new URLSearchParams(window.location.search).get('welcome') === '1'))
  ...
  function goToScreen(nextScreen) {
    if (nextScreen === 'home' && readStorage('ayudate-onboarding-stage', null) === 'configuring') writeStorage('ayudate-onboarding-stage', 'complete')
  ...
  if (screen === 'welcome') {
    return <Welcome initialSelection={readStorage('ayudate-support-selection', ['visual', 'motor'])}
      onContinue={selection => {
        ...
        writeStorage('ayudate-onboarding-stage', 'configuring')
        goToScreen('accessibility')
      }}
      onSkip={() => { writeStorage('ayudate-onboarding-stage', 'complete'); goToScreen('home') }}
    />
  }
  ...
  if (screen === 'accessibility') {
    return <main className={appClass}><AccessibilityScreen preferences={preferences} onPreferences={updatePreferences}
      ...
      onSave={() => { writeStorage('ayudate-onboarding-stage', 'complete'); goToScreen('home') }}
    /></main>
  }
