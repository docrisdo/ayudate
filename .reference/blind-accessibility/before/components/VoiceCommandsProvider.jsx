...
export default function VoiceCommandsProvider({ children }) {
  ...
  useEffect(() => {
    ...
    const timer = setTimeout(() => { say(welcome) }, 0)
    function touch(event) {
      if (welcomeUsed.current || !event.target.closest('.wl-screen') || event.target.closest('button,a,input,label,select,textarea,summary,[role="button"]')) return
  ...
  return <VoiceCommandContext.Provider value={{ configure, registerScope, activate, cancel }}>
    ...
    {visible && speech.commandHost && createPortal(<section className="voice-commands" aria-label="Comandos de voz">
      <div className="voice-command-actions"><button onClick={() => meta.enabled ? actions.current.listen() : activate()} disabled={busy}>{meta.enabled ? 'Comandos de voz' : 'Iniciar guía por voz'}</button>{busy && <button onClick={() => cancel()}>Detener escucha</button>}<button onClick={() => { cancel(false); actions.current.execute({ type: 'help' }) }}>Ayuda de voz</button></div>
