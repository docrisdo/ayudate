...
export default function Welcome({ initialSelection, onContinue, onSkip }) {
  ...
  return <main className="wl-screen">
    ...
    <div className="wl-layout">
      <section className="wl-config" aria-labelledby="wl-title">
        ...
        <fieldset className="wl-options"><legend>Elige los apoyos que necesitas</legend>{supportOptions.map(option => <label key={option.id} className={`wl-option${selection.includes(option.id) ? ' is-selected' : ''}`}><img src={option.icon} alt="" /><span><strong>{option.title}</strong><small>{option.text}</small></span><input type="checkbox" checked={selection.includes(option.id)} onChange={() => toggle(option.id)} aria-label={option.title} /><span className="wl-check" aria-hidden="true">{selection.includes(option.id) && <Icon name="icon-check" />}</span></label>)}</fieldset>
