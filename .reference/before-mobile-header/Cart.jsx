...
export default function Cart({ items, budget, onIncrease, onDecrease, onRemove, onRestore, onNavigate, onRead, voiceSupported }) {
  ...
  function remove(item) { setRemoved(item); onRemove(item.id) }
  return <div className="lists-screen ct-screen">
      ...
      <div className="ct-columns">
        <section className="ct-panel ct-products" aria-label="Productos seleccionados">
          {items.length ? <table className="ct-table"><thead><tr><th scope="col">Producto</th><th scope="col">Cantidad</th><th scope="col"><span className="ct-sr-only">Precio unitario</span></th><th scope="col">Precio</th><th scope="col"><span className="ct-sr-only">Eliminar</span></th></tr></thead><tbody>{items.map(item => <tr key={item.id}>
            ...
            <td><div className="ct-quantity"><button aria-label={`Quitar una unidad de ${item.name}`} onClick={() => { if (item.quantity === 1) remove(item); else onDecrease(item.id) }}>−</button><output aria-label={`Cantidad de ${item.name}`}>{item.quantity}</output><button aria-label={`Agregar una unidad de ${item.name}`} onClick={() => onIncrease(item.id)}>+</button></div></td>
            ...
            <td><button className="ct-remove" aria-label={`Eliminar ${item.name} del carrito`} onClick={() => remove(item)}><Icon name="icon-basura" /></button></td>
          ...
          {removed && <div className="ct-undo" role="status"><span>{removed.name} se eliminó del carrito.</span><button onClick={() => { onRestore(removed.id, removed.quantity); setRemoved(null) }}>Deshacer</button></div>}
    ...
    <dialog ref={dialog} className="ct-dialog" aria-labelledby="ct-dialog-title"><h2 id="ct-dialog-title">{confirmed ? 'Tu carrito está listo para caja' : 'Revisa tu compra'}</h2>{confirmed ? <p>Acude a caja para completar el pago. Tus productos permanecen en el carrito.</p> : <><p>{count} · {units} {units === 1 ? 'unidad' : 'unidades'}</p><ul>{items.map(item => <li key={item.id}><span>{item.quantity} × {item.name}</span><strong>{money(Math.round(item.price * 100) * item.quantity / 100)}</strong></li>)}</ul></>}
      ...
      <div className="ct-dialog-actions">{!confirmed && <button className="lists-button lists-button--blue" onClick={() => setConfirmed(true)}>Confirmar y continuar a caja</button>}<button onClick={() => dialog.current.close()}>{confirmed ? 'Volver al carrito' : 'Seguir revisando'}</button></div>
    </dialog>
  </div>
}
