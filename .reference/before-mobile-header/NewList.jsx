...
function Photo({ product }) { return images[product.id] ? <img className="nl-photo" src={assets.products[images[product.id]]} alt="" /> : <span className="nl-photo" aria-hidden="true" /> }
...
export default function NewList({ products, onNavigate, onSave, onRead, voiceSupported }) {
  ...
  return <div className="lists-screen nl-screen">
    <header className="lists-header"><div className="lists-header-inner">
    ...
    <div className="nl-content">
      ...
      <section className="nl-name nl-panel"><label htmlFor="nl-name">Nombre de la lista</label><input id="nl-name" ref={nameRef} value={name} maxLength={80} onChange={event => setName(event.target.value)} placeholder="Ej. Compra semanal" required /></section>
      <section className="nl-add nl-panel" aria-labelledby="nl-add-title">
        ...
        <div className="nl-input-columns">
          <div className="nl-search"><label htmlFor="nl-search">Buscar producto</label><div className="nl-search-field"><Icon name="icon-buscar" /><input id="nl-search" type="search" ref={searchRef} value={query} onChange={event => setQuery(event.target.value)} placeholder="Escribe el nombre de un producto" autoComplete="off" aria-controls="nl-results" /></div>
          ...
          <div className="nl-bulk"><label htmlFor="nl-bulk">Escribe tus productos (uno por línea)</label><textarea id="nl-bulk" ref={bulkRef} value={bulk} onChange={event => setBulk(event.target.value)} placeholder={'Leche\nHuevos\nManzanas\nArroz\nJabón'} maxLength={5000} /><div className="nl-bulk-help"><small>Un producto por línea. Puedes indicar «2 x Leche».</small><span>{lines}/50</span></div><button className="lists-button" disabled={!lines || lines > 50} onClick={addBulk}><Icon name="icon-plus" />Agregar productos</button></div>
        </div>
        <p className="nl-status" role="status">{message}</p>
      </section>
      <section className="nl-products nl-panel" aria-labelledby="nl-products-title">
        <div className="nl-products-heading"><Icon name="icon-carrito" /><div><h2 id="nl-products-title">Productos en tu lista</h2><p>{selected.length} productos agregados</p></div><button disabled={!selected.length} onClick={() => setClearing(true)}><Icon name="icon-basura" />Vaciar lista</button></div>
        {clearing && <div className="nl-clear"><span>¿Quitar todos los productos de esta lista?</span><button onClick={() => { setQuantities({}); setClearing(false); setMessage('Se vació la lista.') }}>Sí, vaciar</button><button onClick={() => setClearing(false)}>Cancelar</button></div>}
        ...
        <ul className="nl-rows">{selected.map(product => <li key={product.id}>
          ...
          <span className="nl-price"><span className="nl-mobile-label">Precio: </span>{money(product.price)}</span>
          <div className="nl-quantity"><button aria-label={`Reducir cantidad de ${product.name}`} disabled={quantities[product.id] <= 1} onClick={() => changeQuantity(product.id, quantities[product.id] - 1)}>−</button><input type="number" min="1" max="99" step="1" aria-label={`Cantidad de ${product.name}`} value={quantities[product.id]} onChange={event => changeQuantity(product.id, Math.round(Number(event.target.value)))} /><button aria-label={`Aumentar cantidad de ${product.name}`} disabled={quantities[product.id] >= 99} onClick={() => changeQuantity(product.id, quantities[product.id] + 1)}>+</button></div>
          <span className="nl-subtotal"><span className="nl-mobile-label">Subtotal: </span>{money(product.price * quantities[product.id])}</span>
