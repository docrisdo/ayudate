...
const routeZones = [
  { id: 'Entrada', x: 48, y: 92, tone: 'entry' },
  { id: 'Frutas', x: 18, y: 20, tone: 'fruit' },
  { id: 'Panadería', x: 18, y: 48, tone: 'bakery' },
  { id: 'Lácteos', x: 48, y: 20, tone: 'dairy' },
  { id: 'Cereales', x: 76, y: 20, tone: 'cereal' },
  { id: 'Abarrotes', x: 76, y: 58, tone: 'grocery' },
  { id: 'Limpieza', x: 48, y: 58, tone: 'clean' },
  { id: 'Caja', x: 18, y: 86, tone: 'checkout' },
]
...
function App() {
  ...
  const filteredProducts = useMemo(() => {
    ...
    return products.filter((product) => {
      const matchesTerm = !term || `${product.name} ${product.brand} ${product.category} pasillo ${product.aisle}`.toLowerCase().includes(term)
  ...
  const routeZoneSequence = useMemo(() => {
    const categoriesInList = listProducts.map((product) => product.category)
    return ['Entrada', ...new Set(categoriesInList), 'Caja']
  }, [listProducts])
  const activeRouteZones = useMemo(() => {
    return routeZoneSequence
      .map((zoneName, index) => {
        const zone = routeZones.find((item) => item.id === zoneName)
        return zone ? { ...zone, order: index + 1, isCurrent: zoneName === currentStop?.zone } : null
      })
      .filter(Boolean)
  }, [currentStop, routeZoneSequence])
  const routePathPoints = activeRouteZones.map((zone) => `${zone.x},${zone.y}`).join(' ')
  ...
  function readRouteAloud() {
    const routeText = routeStops.map((stop, index) => `${index + 1}. ${stop.label}. ${stop.detail}`).join('. ')
    speak(`Ruta recomendada. ${routeText}`)
  }

  function markProductFound() {
    if (currentStop?.product) addToCart(currentStop.product.id)
    setRouteIndex((index) => Math.min(index + 1, routeStops.length - 1))
  }
  ...
  return (
    <main className={appClass} aria-live="off">
      <div className="app-frame">
          ...
          <section className="page-body">
            <div className="list-layout">
              <section className="panel">
                <PanelTitle icon="list" title="Tus listas guardadas" />
                <div className="inline-form">
                  <label htmlFor="new-list">Nombre de la lista</label>
                  <input id="new-list" value={newListName} onChange={(event) => setNewListName(event.target.value)} placeholder="Ej. Compra semanal" />
                  <button type="button" onClick={createList}>Nueva lista</button>
                </div>
                <div className="list-stack">
                  {lists.map((list) => (
                    <button className={`list-choice ${selectedList?.id === list.id ? 'selected' : ''}`} key={list.id} type="button" aria-pressed={selectedList?.id === list.id} onClick={() => setSelectedListId(list.id)}>
                      <IconBubble icon="cart" />
                      <span>
                        <strong>{list.name}</strong>
                        <small>{list.items.length} productos</small>
                      </span>
                    </button>
                  ))}
                </div>
              ...
              <section className="panel">
                <PanelTitle icon="cart" title={selectedList?.name || 'Lista'} />
                <div className="selected-list">
                  {(selectedList?.items || []).map((itemId) => {
                    const product = products.find((item) => item.id === itemId)
                    return product ? <ProductRow key={product.id} product={product} actionLabel="Quitar" onAction={() => removeProductFromList(product.id)} /> : null
                  })}
                  {selectedList?.items.length === 0 && <p className="empty">Agrega productos desde el catálogo.</p>}
                </div>
                <button className="wide" type="button" onClick={() => setActiveList(selectedList.id)}>Guardar e iniciar compra</button>
              </section>
            </div>
            <section className="panel">
              <PanelTitle icon="search" title="Agrega tus productos" />
              <div className="product-grid">
                {products.map((product) => (
                  <ProductCard key={product.id} product={product} actionLabel="Agregar a lista" onAction={() => addProductToList(product.id)} />
                ))}
              </div>
            </section>
          </section>
          ...
          <section className="page-body">
            <section className="search-panel">
              <label className="search-box" htmlFor="search">
                ...
                <input id="search" value={query} onChange={(event) => setQuery(event.target.value)} placeholder="Buscar producto, categoría o pasillo" />
              </label>
              <button type="button">Buscar</button>
            </section>
            <div className="category-strip" aria-label="Filtrar por categoría">
              {categories.map((item) => (
                <button className={category === item ? 'selected' : ''} key={item} type="button" onClick={() => setCategory(item)}>{item}</button>
              ))}
            </div>
            <p className="result-count">{filteredProducts.length} resultados</p>
            <div className="product-list">
              {filteredProducts.map((product) => (
                <ProductResult key={product.id} product={product} addToList={() => addProductToList(product.id)} addToCart={() => addToCart(product.id)} goToRoute={() => goToScreen('route')} speak={() => speak(`${product.name}. Pasillo ${product.aisle}. Precio ${product.price} pesos.`)} />
              ))}
            </div>
          </section>
        )}
        ...
        {screen === 'route' && (
          <section className="page-body route-layout">
            <section className="panel route-panel">
              <PanelTitle icon="pin" title="Mapa del supermercado" />
              <p>Sigue la ruta para encontrar tus productos.</p>
              <RouteMap zones={routeZones} activeZones={activeRouteZones} routePathPoints={routePathPoints} />
              <div className="map-legend" aria-label="Leyenda del mapa">
                <span><b className="done-dot"></b>Zona en ruta</span>
                <span><b className="now-dot"></b>Zona actual</span>
                <span><b className="path-dot"></b>Tu ruta</span>
              </div>
            </section>
            <aside className="route-sidebar">
              <section className="panel progress-card">
                <PanelTitle icon="cart" title="Tu progreso" />
                <strong>{Math.min(routeIndex, routeStops.length - 1)} de {routeStops.length - 1} paradas</strong>
                <div className="budget-bar" role="progressbar" aria-label="Progreso de la ruta" aria-valuemin="0" aria-valuemax={routeStops.length - 1} aria-valuenow={Math.min(routeIndex, routeStops.length - 1)}>
                  <span style={{ width: `${Math.min((routeIndex / Math.max(routeStops.length - 1, 1)) * 100, 100)}%` }} />
                </div>
              </section>
              <section className="panel next-stop-card">
                <PanelTitle icon="pin" title="Próxima parada" />
                <h2>{currentStop?.zone || 'Caja'} · {currentStop?.detail}</h2>
                {currentStop?.product && <ProductRow product={currentStop.product} actionLabel="Agregar" onAction={() => addToCart(currentStop.product.id)} />}
                <div className="route-actions">
                  <button className="secondary" type="button" onClick={() => speak(currentStop ? `${currentStop.label}. ${currentStop.detail}` : 'Ruta finalizada')} disabled={!voiceSupported}>Escuchar indicación</button>
                  <button type="button" onClick={markProductFound}>Producto encontrado</button>
                  <button className="wide blue" type="button" onClick={() => setRouteIndex((index) => Math.min(index + 1, routeStops.length - 1))}>Siguiente</button>
                </div>
                <p className="assist-note">Indicación en voz alta: “Dirígete a {currentStop?.detail || 'la siguiente parada'}.”</p>
              </section>
              <section className="panel">
                <PanelTitle icon="route" title="Después" />
                <ol className="mini-steps">
                  {routeStops.slice(routeIndex + 1, routeIndex + 3).map((stop) => <li key={stop.id}>{stop.label}</li>)}
                </ol>
                <button className="secondary wide" type="button" onClick={readRouteAloud} disabled={!voiceSupported}>Leer ruta completa</button>
              </section>
            </aside>
          </section>
        )}
        ...
        {screen === 'budget' && (
          <section className="page-body">
            <section className="panel budget-overview">
              <PanelTitle icon="wallet" title="Resumen de tu presupuesto" />
              <label htmlFor="budget">Tu presupuesto</label>
              <input id="budget" type="number" min="0" value={budget} onChange={(event) => updateBudget(Number(event.target.value))} />
              <div className="metric-grid">
                <MetricCard label="Tu presupuesto" value={`$${budget}`} />
                <MetricCard label="Compra actual" value={`$${total}`} tone="orange" />
                <MetricCard label="Disponible" value={`$${remaining}`} tone={remaining >= 0 ? 'green' : 'red'} />
              </div>
              <p>Has utilizado el <strong>{Math.round((total / Math.max(budget, 1)) * 100)}%</strong> de tu presupuesto.</p>
              <div className="budget-bar" role="progressbar" aria-label="Gasto del presupuesto" aria-valuemin="0" aria-valuemax={budget} aria-valuenow={Math.min(total, budget)} aria-valuetext={`Has gastado ${total} pesos de ${budget} pesos`}>
                <span style={{ width: `${Math.min((total / Math.max(budget, 1)) * 100, 100)}%` }} />
              </div>
              <p className={remaining >= 0 ? 'ok' : 'warning'}>{remaining >= 0 ? 'Tu compra está dentro del presupuesto.' : `Se excedió el presupuesto por $${Math.abs(remaining)}.`}</p>
            </section>
            <section className="panel impact-panel">
              <PanelTitle icon="stats" title="Productos con mayor impacto" />
              {cartItems.slice().sort((a, b) => b.price * b.quantity - a.price * a.quantity).map((item) => (
                <ProductRow key={item.id} product={item} actionLabel="Ver" onAction={() => goToScreen('cart')} />
              ))}
              {cartItems.length === 0 && <p className="empty">Agrega productos al carrito para ver el impacto.</p>}
            </section>
          </section>
        )}

        {screen === 'cart' && (
          <section className="page-body cart-layout">
            <section className="panel">
              <PanelTitle icon="cart" title="Productos seleccionados" />
              {cartItems.length === 0 && <p className="empty">Todavía no hay productos en el carrito.</p>}
              {cartItems.map((item) => (
                <div className="cart-row" key={item.id}>
                  <ProductThumb product={item} />
                  <div>
                    <strong>{item.name}</strong>
                    <small>{item.unit} · {item.category}</small>
                  </div>
                  <div className="quantity">
                    <button type="button" onClick={() => removeFromCart(item.id)} aria-label={`Quitar una unidad de ${item.name}`}>-</button>
                    <span>{item.quantity}</span>
                    <button type="button" onClick={() => addToCart(item.id)} aria-label={`Agregar una unidad de ${item.name}`}>+</button>
                  </div>
                  <strong>${item.price * item.quantity}</strong>
                </div>
              ))}
              <button className="secondary" type="button" onClick={() => goToScreen('search')}>Seguir comprando</button>
            </section>
            <aside className="panel summary-card">
              <PanelTitle icon="cart" title="Resumen" />
              <p>{cartItems.length} productos en tu carrito</p>
              <div className="total-row"><span>Total</span><strong>${total}</strong></div>
              <button className="wide blue" type="button" onClick={() => speak(`Total de compra ${total} pesos`)} disabled={!voiceSupported}>Finalizar compra</button>
              <p className="assist-note">Tu presupuesto: {remaining >= 0 ? `te quedan $${remaining}` : `te pasaste por $${Math.abs(remaining)}`}.</p>
            </aside>
          </section>
        )}
...
function RouteMap({ zones, activeZones, routePathPoints }) {
  return (
    <div className="route-map" aria-label="Mapa visual de ruta">
      <div className="shelves" aria-hidden="true">
        {Array.from({ length: 18 }).map((_, index) => <span key={index}></span>)}
      </div>
      <svg className="map-path-svg" viewBox="0 0 100 100" preserveAspectRatio="none" aria-hidden="true">
        <polyline className="map-path-base" points={routePathPoints} />
        <polyline className="map-path-active" points={routePathPoints} />
      </svg>
      {zones.map((zone) => {
        const activeZone = activeZones.find((item) => item.id === zone.id)
        return (
          <div
            className={`map-zone ${zone.tone} ${activeZone ? 'needed' : ''} ${activeZone?.isCurrent ? 'current' : ''}`}
            key={zone.id}
            style={{ '--zone-x': `${zone.x}%`, '--zone-y': `${zone.y}%`, '--zone-index': activeZone ? activeZone.order - 1 : 0 }}
            aria-label={`${zone.id}${activeZone ? `, parada ${activeZone.order}` : ''}`}
          >
            {activeZone && <b>{activeZone.order}</b>}
...
function ProductCard({ product, actionLabel, onAction }) {
  return (
    <article className="product-card">
      ...
      <div>
        <h3>{product.name}</h3>
        <p>{product.category} · Pasillo {product.aisle}</p>
      ...
      <button type="button" onClick={onAction} disabled={!product.available} aria-label={`${actionLabel}: ${product.name}${product.available ? '' : '. Producto agotado'}`}>{actionLabel}</button>
    </article>
  )
}

function ProductRow({ product, actionLabel, onAction }) {
  return (
    <div className="product-row">
      <ProductThumb product={product} />
      <span>{product.name}<small>Pasillo {product.aisle} · {product.category}</small></span>
      <strong>${product.price}</strong>
