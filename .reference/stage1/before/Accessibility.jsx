...
export default function Accessibility({ preferences, onPreferences, product, quantity, onAddToCart, onNavigate, onRead, voiceSupported, onSave }) {
  ...
  return <div className={`lists-screen ac-screen${preferences.visualHints ? ' ac-hints' : ''}`}>
    ...
    <main className="ac-content" id="contenido-principal" tabIndex={-1}>
      <section className="ac-hero" aria-labelledby="ac-title"><div><h1 id="ac-title"><span className="a11y-sr-only">Configuración de accesibilidad. </span>Haz AYÚ<span>DATE más cómodo</span> para ti</h1><p>Puedes cambiar estas opciones cuando quieras.</p></div><div className="ac-banner" aria-hidden="true"><p>Una experiencia<br />de compra para<br />tu día a día ♡</p><img src={assets.illustrations.productos_frescos} alt="" /></div></section>
