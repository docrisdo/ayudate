...
function App() {
  ...
  const [preferences, setPreferences] = useState(() => ({ ...defaultPreferences, ...readStorage('ayudate-preferences', defaultPreferences), voice: false, voiceCommands: false }))
  ...
  const announceScreen = useEffectEvent(() => {
    if (voiceNavigation.current) { voiceNavigation.current = false; return }
    stop()
    if (screen !== 'welcome' && preferences.voice && preferences.autoRead) speak(screenIntroductions[screen])
  })
  useEffect(() => {
  ...
  function updatePreferences(next) {
    ...
    if (preferences.voice && !next.voice) stop()
  }
  ...
  useEffect(() => {
    voice.configure({ enabled: preferences.voiceCommands, screen, onEnable: () => updatePreferences({ ...preferences, voiceCommands: true }), onCommand: handleVoiceCommand })
  })
  ...
  if (screen === 'accessibility') {
    return <div className={appClass}><AccessibilityScreen preferences={preferences} onPreferences={updatePreferences}
